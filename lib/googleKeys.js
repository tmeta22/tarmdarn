...
export async function addKey(key, label) {
  ...
  // Google keys today are 39 characters ("AIza" + 35), but that length is
  // not guaranteed, so only the prefix and character set are checked here.
  // The live probe below is what actually decides whether a key works —
  // rejecting on length risked turning away perfectly good keys.
  if (!/^AIza[0-9A-Za-z_-]{30,}$/.test(trimmed)) {
    return {
      error:
        "That doesn't look like a Google API key — they start with AIza and contain no spaces or punctuation.",
    };
  }

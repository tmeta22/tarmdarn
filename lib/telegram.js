...
export function formatCheckSummary({
  ...
  if (skipped > 0) {
    // A sweep that ran out of rounds has not completed, so say which of the
    // two actually happened instead of claiming both at once.
    lines.push(
      rounds > 1
        ? `Stopped after ${rounds} runs with ${skipped} still to check.`
        : `${skipped} left for the next run to stay inside the time limit.`
    );
  } else if (rounds > 1) {
    lines.push(`Full sweep completed in ${rounds} runs.`);
  }

...
export function formatCheckSummary({
  ...
  const failed = results.filter((r) => r.error && !r.gone && !r.retryable);
  const limited = results.filter((r) => r.retryable);
  ...
  if (limited.length) headline.push(`${limited.length} rate limited`);

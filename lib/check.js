 ...
 *
 * The next round runs for its full budget (~45s). Holding this instance open
 * until that response arrives would nest the rounds and keep every parent
 * alive past its own 60s limit — which is what silently cut a production
 * sweep short after five rounds, leaving 109 places unchecked and sending no
 * notification at all. Staying alive just long enough for the request to
 * leave is enough; from then on the next round is its own invocation.
 *

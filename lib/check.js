 ...
 *
 * @param since Marks the start of the sweep. Anything already checked after
 *   it counts as done, which is how a sweep knows it has finished — note that
 *   `skipped` alone cannot tell you, because it only ever reports what fell
 *   outside this round's budget, which for a table larger than one round can
 *   do is always most of it.
 * @returns {Promise<{results: Array, skipped: number, remaining: number}>}
 *   `skipped` is what this round's budget did not reach; `remaining` is what
 *   the whole sweep still has left.
 */
export async function runPlaceCheck({ budgetMs = BUDGET_MS, since = null } = {}) {
  ...
  const alreadyDone =
    sweepStart === null
      ? 0
      : list.filter(
          (p) => p.last_checked_at && new Date(p.last_checked_at).getTime() >= sweepStart
        ).length;
  ...
  return {
    results,
    skipped: list.length - results.length,
    remaining: Math.max(0, list.length - alreadyDone - results.length),
  };
}
